...
try{
	...
	if("low".equals(difficulty)){
		...
		count = listdao.getLow_Count(); // low게시물의 수 세기
		
		if(count > 0){
			recipeList = listdao.getLow(startRow, endRow);//첫 게시물부터 마지막 게시글까지 가져오기
		}
...
%> <p /><br /><center /> <%
		...
		if(startPage>5){%>
		<a href="/kook/list/low.jsp?difficulty=<%=difficulty %>&pageNum=<%= startPage - 5 %>">[이전]</a>
		<%}
		for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="/kook/list/low.jsp?difficulty=<%= difficulty %>&pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="/kook/list/low.jsp?difficulty=<%= difficulty %>&pageNum=<%= startPage + 5 %>">[다음]</a>
<%
        }
    }
