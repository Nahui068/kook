...
<% 
	...
	if("dessert".equals(category)){
		...
		count = listdao.getDessert_Count();
		
		if(count > 0){
			recipeList = listdao.getDessert(startRow, endRow);
		}
...
%> <p /><br /><center /> <%
		...
		if(startPage>5){%>
		<a href="/kook/list/dessert.jsp?category=<%= category %>&pageNum=<%= startPage - 5 %>">[이전]</a>
		<%}
		for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="/kook/list/dessert.jsp?category=<%= category %>&pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="/kook/list/dessert.jsp?category=<%= category %>&pageNum=<%= startPage + 5 %>">[다음]</a>
<%
        }
    }
