...
try{
	...
	if("stew_soup".equals(category)){
		...
		count = listdao.getStew_soup_Count(); //main_dish 게시글의 수
		
		if(count > 0){
			recipeList = listdao.getStew_soup(startRow, endRow);//첫 게시물부터 마지막 게시글까지 가져오기
		}
		
	}
%>
<title>Stew & Soup</title>   
<div class="all">
<br>
...
%> <p /><br /><center /> <%
		...
		if(startPage>5){%>
		<a href="/kook/list/stew_soup.jsp?category=<%= category %>&pageNum=<%= startPage - 5 %>">[이전]</a>
		<%}
		for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="/kook/list/stew_soup.jsp?category=<%= category %>&pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="/kook/list/stew_soup.jsp?category=<%= category %>&pageNum=<%= startPage + 5 %>">[다음]</a>
<%
        }
    }
